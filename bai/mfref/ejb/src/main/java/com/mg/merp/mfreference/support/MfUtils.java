...
public class MfUtils {
  ...
  public static BigDecimal calculateMaterialQuan(QuantityRateFlag quantityRateFlag, BigDecimal quan, BigDecimal timeOper, BigDecimal scrapFactor, BigDecimal lotQuan) {
    switch (quantityRateFlag) {
      case FIXED:
        return quan.divide(BigDecimal.ONE.subtract(scrapFactor).multiply(lotQuan));
      case TIME:
        return timeOper.multiply(quan).divide(BigDecimal.ONE.subtract(scrapFactor));
      case UNIT:
        return quan.divide(BigDecimal.ONE.subtract(scrapFactor));
      default:
  ...
  /**
   * настройка дат действий объектов производства, производится поиск полей с именами EffOffDate и
   * EffOnDate, если данные поля не установлены, то приводятся к стандартным значениям
   *
