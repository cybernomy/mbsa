...
public class ResourceFamilyServiceBean extends AbstractPOJODataBusinessObjectServiceBean<ResourceFamily, Integer> implements ResourceFamilyServiceLocal {
  ...
  /**
   * Если на каком-то уровне группа A является родительской для группы B, то для этого уровня нельзя
   * добавить запись, где группа A была бы для группы B дочерней
   *
   * @param resourceFamily -	сущность
   * @return -	<code>false</code> на данном уровне планирования есть запись в которой значение
   * родительской группы совпадает со значением дочерней группы у текущей сущности и значение
   * дочерней группы ресурсов совпадает с родительской у текущей сущности
   */
