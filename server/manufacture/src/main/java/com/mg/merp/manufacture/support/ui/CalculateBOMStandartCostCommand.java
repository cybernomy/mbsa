/*
 * CalculateBOMStandartCostCommand.java
 *
 ...
 * @version $Id: CalculateBOMStandartCostCommand.java,v 1.2 2008/08/15 14:40:50 sharapov Exp $
 */
public class CalculateBOMStandartCostCommand implements MenuCommand {
    ...
    Dialogs.inputQuery(Messages.getInstance().getMessage(Messages.INPUT_DATE), Messages.getInstance().getMessage(Messages.ACTUALITY_DATE), DateTimeUtils.nowDate(), new InputQueryDialogListener<Date>() {
      ...
      public void inputPerformed(Date value) {
        ((CostProcessorServiceLocal) ApplicationDictionaryLocator.locate().getBusinessService(CostProcessorServiceLocal.SERVICE_NAME))
            .calculateBOMStandartCost(value);
  ...
  public boolean isPermitted() {
    return SecurityUtils.tryCheckPermission(new BusinessMethodPermission(CostProcessorServiceLocal.SERVICE_NAME, "calculateBOMStandartCost")); //$NON-NLS-1$
  }
