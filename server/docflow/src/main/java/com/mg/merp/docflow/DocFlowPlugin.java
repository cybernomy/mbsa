...
public interface DocFlowPlugin {
  ...
  /**
   * показать в пользовательском интерфейсе результат выполненного этапа ДО. <strong>Вызов данного
   * метода возможен только если текущий пользователь является интерактивным.</strong>
   *
   * @param docHeadState состояние этапа ДО
   */
