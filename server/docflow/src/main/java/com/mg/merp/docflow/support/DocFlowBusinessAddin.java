...
/**
 * Базовый класс BAi отработки (отката) документа в документообороте. Класс BAi должен
 ...
 * прервать выполнение документооборота необходимо вызвать {@link #abort()}. Класс BAi
 * предназначенный для отработки может реализовывать методы {@link #doGetDocActionResultTextRepresentation(DocHeadState)} и
 * {@link #doShowDocActionResult(DocHeadState)} предназначенные для вывода в пользовательском
 * интерфейсе результатов выполнения этапа ДО.
 * 
...
public abstract class DocFlowBusinessAddin extends BaseDocFlowBusinessAddin implements com.mg.merp.docflow.BAiDocFlowPluginViewer {
	...
	/**
	 * реализация генерации текстового представления результата выполнения этапа ДО,
	 * конкретная реализация этапа ДО должна переопределить данный метод если есть возможность
	 * показать результат в строковом виде, данная строка будет выведена в форме истории ДО
	 * 
	 ...
	 * показать данный документ в UI), данный метод будет вызван при выборе пункта меню "Показать результат этапа"
	 * в истории ДО
	 * 
	 * @param docHeadState	состояние ДО
	 */
