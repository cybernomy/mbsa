...
public class DocFlowPluginInvokeParams {
  ...
  /**
   * получить признак "тихого" выполнения ДО, в случае если значение <code>true</code>, то этап ДО
   * не должен взаимодействовать с пользовательским интерфейсом, если существует потребность во
   * взаимодействии с пользователем, то выполнение этапа должно генерировать ИС {@link
   * com.mg.merp.docflow.SilentException}
   *
   ...
   * полученной при отработке документа, данная информация будет доступна при выполнении отката
   *
   ...
   * версиями, при разработке новых этапов ДО рекомендуется использовать {@link
   * #getHeadStateValue()}
   *
   * @return состояние 1
   */
