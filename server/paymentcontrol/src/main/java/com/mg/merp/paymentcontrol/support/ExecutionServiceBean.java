...
/**
 * –еализаци€ бизнес-компонента "»сполнение об€зательств"
 * 
...
public class ExecutionServiceBean extends com.mg.framework.generic.AbstractPOJODataBusinessObjectServiceBean<Execution, Integer> implements ExecutionServiceLocal {
	...
	private void doExecuteLiability(Liability liability, PmcResource pmcResource, Date planDate, BigDecimal sumCur) {
		short transferKind = 0; // Ќ≈ внутреннее перемещение средств
	...
	private void internalTransferResourses(TransferParams transferParams, Integer versionId) {
		...
		// об€зательство по расходу (expense)
		...
		// исполнение об€зательства по расходу (expense)
		...
		// об€зательство по приходу (income)
		...
		// исполнение об€зательства по приходу (income)
	...
	/**
	 * —оздать об€зательство с использованием образца
	 * @param resource - средство платежа
	 * @param dateToExecute - дата иполнени€
	 * @param sumCur - сумма
	 * @param liabilityModel - образец об€зательства
	 * @param transferKind - признак перемещени€ ресурсов
	 * @param version - верси€ планировани€
	 * @return об€зательство
	 */
	...
	private DocHead createDocumentByExecution(Execution execution, StringBuilder detailReportText) {
		...
		if(MathUtils.compareToZero(((BankDocumentModel) docHeadModel).getNds2Rate()) == 1) {
			...
			if(MathUtils.compareToZero(((BankDocumentModel) docHeadModel).getNds1Rate()) == 1) {
