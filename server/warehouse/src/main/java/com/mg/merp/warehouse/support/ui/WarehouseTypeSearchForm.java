 ...
 */
public class WarehouseTypeSearchForm extends AbstractSearchForm {


  private final String INIT_QUERY_TEXT = "select %s from WarehouseType wt %s "; //$NON-NLS-1$
  ...
  protected void doOnRun() {
    ...
    table = new DefaultTableController(new DefaultMaintenanceEJBQLTableModel() {
      ...
      protected Set<TableEJBQLFieldDef> getDefaultFieldDefsSet() {
        ...
        result.add(new TableEJBQLFieldDef(WarehouseType.class, "Id", "wt.Id", true)); //$NON-NLS-1$ //$NON-NLS-2$
        result.add(new TableEJBQLFieldDef(WarehouseType.class, "Code", "wt.Code", false)); //$NON-NLS-1$ //$NON-NLS-2$
        result.add(new TableEJBQLFieldDef(WarehouseType.class, "Name", "wt.Name", false)); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  protected PersistentObject[] getSearchedEntities() {
    ...
    for (int i = 0; i < selectedIds.length; i++)
      selectedEntities[i] = persistentManager.find(WarehouseType.class, selectedIds[i]);
