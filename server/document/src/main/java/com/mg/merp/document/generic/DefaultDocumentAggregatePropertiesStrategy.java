...
/**
 * —тандартна€ реализаци€ стратегии изменени€ агрегирующих свойств документа
 * 
...
public class DefaultDocumentAggregatePropertiesStrategy extends
	...
	protected void doAdjustDocHead(DocHead docHead) {
		...
		// округлим сумму в валюте до точности, установленной в конфигурации модул€
	...
	protected void doCalculate() {
		//нет измен€емых спецификаций, не измен€ем заголовок
		...
		//при модификации документа не провер€ем ƒќ, иначе невозможно модифицировать
		//документ штатными методами сервисов после прохождени€ ƒќ, проверку на прохождение ƒќ
		//возлагаем на прикладной код, там где это требуетс€, например при интерактивном
		...
		//включим спецификации из списка, т.к. они не участвовали в загрузке агрегирующих значений (кроме удал€емых)
		if (!isRemove)
		...
		//проставим агрегирующие значени€ в документе
		...
		//сбросим изменени€ заголовка документа в базу, см. https://issues.m-g.ru/bugzilla/show_bug.cgi?id=4985
