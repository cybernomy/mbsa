...
public class DocumentProcessorImpl implements DocumentProcessor {
   ...
   * Создание документа
   *
   * @param params   данные этапа документооборота
   * @param kind     признак создания документа (0 - создать документ на основании, 1 - создать
   *                 документ на комплектующие и т.д.)
   * @param listener слушатель на событие редактирование документа
   */
   ...
   *
   * @param kind признак создания документа (0 - создать документ на основании, 1 - создать документ
   *             на комплектующие и т.д.)
   * @return стратегия создания документа
   */
  @SuppressWarnings("unchecked")
  private CreateDocumentBasisOf getCreateDocumentStrategy(int kind) {
    switch (kind) {
      case 0:
        return CreateDocumentBasisOfLocator.locate();
      case 1:
        return CreateDocOnComponentsLocator.locate();
      default:
        return CreateDocumentBasisOfLocator.locate();
    }
  }
