...
public abstract class DocumentServiceBean<T extends com.mg.merp.document.model.DocHead, ID extends Serializable, M extends DocumentPattern>
  ...
  /**
   * получить идентификатор раздела документа, должен быть переопределен в классе реального
   * документа
   *
   ...
   *
   * @param <T1>   Тип документа-источника
   * @param <T2>   Тип модели документа
   * @param srcDoc Документ-источник
