 ...
 */
public class ContractTypeSearchForm extends AbstractSearchForm {

  private final String INIT_QUERY_TEXT = "select %s from ContractType ct %s "; //$NON-NLS-1$
  ...
  protected void doOnRun() {
    ...
    table = new DefaultTableController(new DefaultMaintenanceEJBQLTableModel() {
      ...
      protected Set<TableEJBQLFieldDef> getDefaultFieldDefsSet() {
        ...
        result.add(new TableEJBQLFieldDef(ContractType.class, "Id", "ct.Id", true)); //$NON-NLS-1$ //$NON-NLS-2$
        result.add(new TableEJBQLFieldDef(ContractType.class, "Code", "ct.Code", false)); //$NON-NLS-1$ //$NON-NLS-2$
        result.add(new TableEJBQLFieldDef(ContractType.class, "Name", "ct.Name", false)); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  protected PersistentObject[] getSearchedEntities() {
    ...
    for (int i = 0; i < selectedIds.length; i++)
      selectedEntities[i] = persistentManager.find(ContractType.class, selectedIds[i]);
