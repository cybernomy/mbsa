...
public interface Document<T extends DocHead, ID extends Serializable, M extends DocumentPattern> extends DataBusinessObjectService<T, ID>, AttachmentHandler {
  ...
  /**
   * расчет атрибутов документа, при использовании штатных методов создания и изменения вызов
   * данного метода не требуется, если атрибут {@link com.mg.merp.document.model.DocHead#isAdjusted()
   * Adjusted} установлен в <code>true</code>, то расчет производится не будет, т.о. возможно
   * изменить стандартное поведение системы
   *
