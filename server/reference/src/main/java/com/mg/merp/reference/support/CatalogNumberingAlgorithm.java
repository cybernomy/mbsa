 ...
 * реализовывать следующий метод <code>protected String doPerform() throws Exception</code>. Метод
 * возвращает сформированный код позиции КТУ.
 *
...
public abstract class CatalogNumberingAlgorithm extends AbstractBusinessAddin<String> {
  ...
  /**
   * Возвращает код, сформированный процедурой по-умолчанию. Данная процедура формирует код из
   * последовательности кодов папок, составляющих путь от папки с идентификатором RootId до текущей
   * папки, разделенных символом Separator. Последовательность завершается номером элемента в данной
   * папке.
   *
   ...
   * @param numWidth    определяет ширину заключительного элемента кода (номера элемента). Если
   *                    число символов в номере меньше NumWidth, то номер дополняется слева нулями
   */
   ...
   *
   * @param name наименование атрибута, может иметь следующие значения: <dl> <dt>Id
   *             <dd>идентификатор <dt>FolderId <dd>идентификатор папки <dt>Code <dd>код
   *             <dt>FullName <dd>наименование <dt>BarCode <dd>штрих-код <dt>PLUCode <dd>код PLU
   *             <dt>OKDPCode <dd>код ОКДП <dt>Measure1_Id <dd>идентификатор основной единицы
   *             измерения <dt>Measure2_Id <dd>идентификатор дополнительной единицы измерения
   *             <dt>Weight <dd>вес <dt>Volume <dd>объем <dt>Articul <dd>артикул </dl>
