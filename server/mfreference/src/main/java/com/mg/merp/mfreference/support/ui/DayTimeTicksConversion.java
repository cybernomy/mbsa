...
/**
 * Преобразование тиков в часы. Данный конвертер преобразует количество тиков в количество часов и минут,
 * а не во время, поэтому используется календарь GMT, чтобы не было влияния временных зон, DateFormat
 * используется исключительно для удобства преобразования строк, чтобы в UI часы и минуты выглядели
 * в локали пользователя
 * 
 * @author leonova
