...
/**
 * –еализаци€ бизнес-компонента "–асчетные периоды" 
 * 
 * @author leonova
...
public class CalcPeriodServiceBean extends AbstractPOJODataBusinessObjectServiceBean<CalcPeriod, Integer> implements CalcPeriodServiceLocal {
	...
	/**
	 * ѕроверить корректность диапазона дат
	...
	/**
	 * ѕроверить пересечение даным периодом других периодов
	 * @param calcPeriod - расчетный период
	...
	protected boolean isPeriodCross(CalcPeriod calcPeriod) {
		...
		// при редактировании периода исключаем измен€емый период из списка существующих периодов
