...
/**
 * Базовый класс BAi формирования набора данных для генератора отчётов. Класс алгоритма должен
 * реализовывать следующий метод <code>protected void doFillDataSet()</code> и возвращать заполненый
 * набор данных, представляющий из себя {@link com.mg.framework.api.dataset.DataSet DataSet}.
 * Алгоритм имеет доступ к параметрам набора данных через метод {@link
 * #getDataSetParameter(String)}. Доступ к идентификатору выбраных сущностей осуществляется вызовом
 * метода {@link #getEntityIds()}, возвращающего массив объектов типа java.io.Serializable.
 *
 * <p>Пример1:
 *
 * <pre>
...
public abstract class ReportBusinessAddin extends AbstractBusinessAddin<DataSet> {
  ...
  /**
   * контекст выполнения отчета, в данном контексте возможно сохранять данные доступные для всех BAi
   * участвующих в формировании отчета
   */
   ...
   * получение идентификатора сущности для которой генерируется отчет
   *
   * @return идентификатор первой сущности из списка идентификаторов или <code>null</code> если
   * список пустой
   */
