...
/**
 * Контроллер браузера расчетных ведомостей
 *
 * @author leonova
...
public class PayRollBr extends DefaultPlainBrowseForm {
  ...
  /**
   * Обработчик пункта КМ "Создать платежную ведомость"
   *
   * @param event - событие
   */
  ...
  /**
   * Обработчик пункта КМ "Создать свод н/у по аналитике"
   *
   * @param event - событие
   */
  public void onActionCreateFeeSummary(WidgetEvent event) {
    ...
    if (selectedPayRollIds != null && selectedPayRollIds.length > 0) {
      ...
      final CreateFeeSummaryDlg сreateFeeSummaryDlg = (CreateFeeSummaryDlg) UIProducer.produceForm(CREATE_FEESUMARY_FORM_NAME);
      сreateFeeSummaryDlg.addOkActionListener(new FormActionListener() {

        /* (non-Javadoc)
         * @see com.mg.framework.api.ui.FormActionListener#actionPerformed(com.mg.framework.api.ui.FormEvent)
         */
        public void actionPerformed(FormEvent event) {
          FeeSummaryHead feeSummaryHead = payRollService.createFeeSummary(selectedPayRollIds, сreateFeeSummaryDlg.getFeeSummaryPattern());
          if (feeSummaryHead != null && сreateFeeSummaryDlg.isShowCreatedDocument())
            MaintenanceHelper.edit(feeSummaryHeadService, feeSummaryHead.getId(), null, null);
        }
      });
      сreateFeeSummaryDlg.setDocSection(feeSummaryHeadService.getDocSection());
      сreateFeeSummaryDlg.execute();
    }
  }
