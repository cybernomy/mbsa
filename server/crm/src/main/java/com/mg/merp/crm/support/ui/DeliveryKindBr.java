 ...
 */
public class DeliveryKindBr extends DefaultPlainBrowseForm {
  private final String INIT_QUERY_TEXT = "select delkind.Id";
  private String fieldsList;

  @Override
  protected String createQueryText() {
    return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from DeliveryKind delkind order by delkind.Id"), service, "delkind.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  protected MaintenanceTableModel createModel() {
    return new DefaultMaintenanceEJBQLTableModel() {
      ...
      protected void createColumnsModel() {
        ...
        addColumnDef(DeliveryKind.class, "Id", null); //$NON-NLS-1$
        if (fields.contains("Code")) {
          fieldsList = fieldsList.concat(", delkind.Code");
          addColumnDef(DeliveryKind.class, "Code", null); //$NON-NLS-1$
        }
        if (fields.contains("Name")) {
          fieldsList = fieldsList.concat(", delkind.Name");
          addColumnDef(DeliveryKind.class, "Name", null); //$NON-NLS-1$
        }
