 ...
 */
public class OfferStatusBr extends DefaultPlainBrowseForm {
	private final String INIT_QUERY_TEXT = "select offstatus.Id";
	private String fieldsList;
	
	@Override
	protected String createQueryText() {
		return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from OfferStatus offstatus order by offstatus.Id"), service, "offstatus.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected MaintenanceTableModel createModel() {
		return new DefaultMaintenanceEJBQLTableModel() {
			...
			protected void createColumnsModel() {
				...
				addColumnDef(OfferStatus.class, "Id", null); //$NON-NLS-1$
				if (fields.contains("Code")) {
					fieldsList = fieldsList.concat(", offstatus.Code");
					addColumnDef(OfferStatus.class, "Code", null); //$NON-NLS-1$
				}
				if (fields.contains("Name")) {
					fieldsList = fieldsList.concat(", offstatus.Name");
					addColumnDef(OfferStatus.class, "Name", null); //$NON-NLS-1$
				}
