 ...
 */
public class OfferForecastBr extends DefaultPlainBrowseForm {
	private final String INIT_QUERY_TEXT = "select offfor.Id";
	private String fieldsList;
	
	@Override
	protected String createQueryText() {
		return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from OfferForecast offfor order by offfor.Id"), service, "offfor.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected MaintenanceTableModel createModel() {
		return new DefaultMaintenanceEJBQLTableModel() {
			...
			protected void createColumnsModel() {
				...
				addColumnDef(OfferForecast.class, "Id", null); //$NON-NLS-1$
				if (fields.contains("Code")) {
					fieldsList = fieldsList.concat(", offfor.Code");
					addColumnDef(OfferForecast.class, "Code", null); //$NON-NLS-1$
				}
				if (fields.contains("Name")) {
					fieldsList = fieldsList.concat(", offfor.Name");
					addColumnDef(OfferForecast.class, "Name", null); //$NON-NLS-1$
				}				
