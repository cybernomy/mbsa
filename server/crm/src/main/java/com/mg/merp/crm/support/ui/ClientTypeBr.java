 ...
 */
public class ClientTypeBr extends DefaultPlainBrowseForm {
  private final String INIT_QUERY_TEXT = "select cltype.Id";
  private String fieldsList;

  @Override
  protected String createQueryText() {
    return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from ClientType cltype order by cltype.Id"), service, "cltype.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  protected MaintenanceTableModel createModel() {
    return new DefaultMaintenanceEJBQLTableModel() {
      ...
      protected void createColumnsModel() {
        ...
        addColumnDef(ClientType.class, "Id", null); //$NON-NLS-1$
        if (fields.contains("Code")) {
          fieldsList = fieldsList.concat(", cltype.Code");
          addColumnDef(ClientType.class, "Code", null); //$NON-NLS-1$
        }
        if (fields.contains("Name")) {
          fieldsList = fieldsList.concat(", cltype.Name");
          addColumnDef(ClientType.class, "Name", null); //$NON-NLS-1$
        }
