 ...
 */
public class ClientRankBr extends DefaultPlainBrowseForm {
	private final String INIT_QUERY_TEXT = "select clrank.Id";
	private String fieldsList;
	
	@Override
	protected String createQueryText() {
		return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from ClientRank clrank order by clrank.Id"), service, "clrank.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected MaintenanceTableModel createModel() {
		return new DefaultMaintenanceEJBQLTableModel() {
			...
			protected void createColumnsModel() {
				...
				addColumnDef(ClientRank.class, "Id", null); //$NON-NLS-1$
				if (fields.contains("Code")) {
					fieldsList = fieldsList.concat(", clrank.Code");
					addColumnDef(ClientRank.class, "Code", null); //$NON-NLS-1$
				}
				if (fields.contains("Name")) {
					fieldsList = fieldsList.concat(", clrank.Name");
					addColumnDef(ClientRank.class, "Name", null); //$NON-NLS-1$
				}
