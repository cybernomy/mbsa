 ...
 */
public class BranchBr extends DefaultPlainBrowseForm {
  private final String INIT_QUERY_TEXT = "select branch.Id";
  private String fieldsList;

  @Override
  protected String createQueryText() {
    return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from Branch branch order by branch.Id"), service, "branch.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  protected MaintenanceTableModel createModel() {
    return new DefaultMaintenanceEJBQLTableModel() {
      ...
      protected void createColumnsModel() {
        ...
        addColumnDef(Branch.class, "Id", null); //$NON-NLS-1$
        if (fields.contains("Code")) {
          fieldsList = fieldsList.concat(", branch.Code");
          addColumnDef(Branch.class, "Code", null); //$NON-NLS-1$
        }
        if (fields.contains("Name")) {
          fieldsList = fieldsList.concat(", branch.Name");
          addColumnDef(Branch.class, "Name", null); //$NON-NLS-1$
        }
