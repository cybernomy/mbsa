 ...
 */
public class PersonTitleBr extends DefaultPlainBrowseForm {
	private final String INIT_QUERY_TEXT = "select peritle.Id";
	private String fieldsList;
	
	@Override
	protected String createQueryText() {
		return DatabaseUtils.embedAddinFieldsBrowseEJBQL(INIT_QUERY_TEXT.concat(fieldsList).concat(" from PersonTitle peritle order by peritle.Id"), service, "peritle.Id", getFieldsSet()); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected MaintenanceTableModel createModel() {
		return new DefaultMaintenanceEJBQLTableModel() {
			...
			protected void createColumnsModel() {
				...
				addColumnDef(PersonTitle.class, "Id", null); //$NON-NLS-1$
				if (fields.contains("Code")) {
					fieldsList = fieldsList.concat(", peritle.Code");
					addColumnDef(PersonTitle.class, "Code", null); //$NON-NLS-1$
				}
				if (fields.contains("Name")) {
					fieldsList = fieldsList.concat(", peritle.Name");
					addColumnDef(PersonTitle.class, "Name", null); //$NON-NLS-1$
				}
