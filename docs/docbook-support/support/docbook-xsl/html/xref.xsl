...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- ********************************************************************
       $Id: xref.xsl,v 1.1 2005/02/05 11:59:07 krivopoustov Exp $
       ********************************************************************
        ...
        <xsl:when test="@targetdoc and @targetptr">
          ...
          <xsl:choose>
            ...
            <xsl:when test="not($target.database/targetset)">
              <xsl:message>Olink error: could not open target database <xsl:value-of
            ...
            <xsl:when test="$targetdoc.key = ''">
              <xsl:message>Olink error: document id
                <xsl:value-of select="$seek.targetdoc"/> not in target database.
              </xsl:message>
            </xsl:when>
            ...
            <xsl:when test="$targetptr.key = ''">
              ...
              <xsl:choose>
                <xsl:when test="$document.root = ''">
                  <xsl:message>Olink error: could not open data file for document id '<xsl:value-of
                      select="$seek.targetdoc"/>'.
                  </xsl:message>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:message>Olink error: targetptr
                    <xsl:value-of select="$seek.targetptr"/> not found in document id <xsl:value-of
                        select="$seek.targetdoc"/>.
                  </xsl:message>
                </xsl:otherwise>
              </xsl:choose>
    ...
    <xsl:choose>
      ...
      <xsl:when test="@targetdoc and @targetptr">
        ...
        <xsl:variable name="xref.text">
          <xsl:for-each select="$target.database">
            <xsl:value-of
                select="key('targetptr-key', concat($seek.targetdoc, '/', $seek.targetptr))/xreftext"/>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:message>Olink error: no generated text for targetdoc/targetptr = <xsl:value-of
                select="@targetdoc"/>/<xsl:value-of select="@targetptr"/>
            </xsl:message>
            <xsl:text>????</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
