...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="doc exsl set h exslt">

  <!-- ********************************************************************
       $Id: profile-htmlhelp-common.xsl,v 1.1 2005/02/05 11:59:10 krivopoustov Exp $
       ******************************************************************** -->
  ...
  <xsl:template match="/">
    ...
    <xsl:if
        test="($rootid = '' and //processing-instruction('dbhh')) or                 ($rootid != '' and $profiled-nodes//*[@id=$rootid]//processing-instruction('dbhh'))">
  ...
  <xsl:template name="hhp-main">
    ...
    <xsl:variable name="xnavigation">
      ...
      <xsl:call-template name="toHex">
        <xsl:with-param name="n"
                        select="9504 + $htmlhelp.show.menu * 65536                                           + $htmlhelp.show.advanced.search * 131072                                           + $htmlhelp.show.favorities * 4096                                           + (1 - $htmlhelp.show.toolbar.text) * 64                                           + $htmlhelp.remember.window.position * 262144"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="xbuttons">
      ...
      <xsl:call-template name="toHex">
        <xsl:with-param name="n"
                        select="0 + $htmlhelp.button.hideshow * 2                                        + $htmlhelp.button.back * 4                                        + $htmlhelp.button.forward * 8                                        + $htmlhelp.button.stop * 16                                        + $htmlhelp.button.refresh * 32                                        + $htmlhelp.button.home * 64                                        + $htmlhelp.button.options * 4096                                        + $htmlhelp.button.print * 8192                                        + $htmlhelp.button.locate * 2048                                        + $htmlhelp.button.jump1 * 262144                                        + $htmlhelp.button.jump2 * 524288                                        + $htmlhelp.button.next * 2097152                                        + $htmlhelp.button.prev * 4194304                                        + $htmlhelp.button.zoom * 1048576"/>
      </xsl:call-template>
    </xsl:variable>
<xsl:text>[OPTIONS]
</xsl:text>
...
[FILES]
    ...
    <xsl:if
        test="($htmlhelp.force.map.and.alias != 0) or                ($rootid = '' and //processing-instruction('dbhh')) or               ($rootid != '' and $profiled-nodes//*[@id=$rootid]//processing-instruction('dbhh'))">
  <xsl:text>
[ALIAS]
...
&lt;/HTML&gt;</xsl:text>
  ...
  <xsl:template match="part|reference|preface|chapter|bibliography|appendix|article|glossary"
    ...
    <xsl:if test="reference|preface|chapter|appendix|refentry|section|sect1|bibliodiv">
      <xsl:text disable-output-escaping="yes">&lt;UL&gt;</xsl:text>
      <xsl:apply-templates
          select="reference|preface|chapter|appendix|refentry|section|sect1|bibliodiv" mode="hhc"/>
      <xsl:text disable-output-escaping="yes">&lt;/UL&gt;</xsl:text>
    </xsl:if>
  </xsl:template>
