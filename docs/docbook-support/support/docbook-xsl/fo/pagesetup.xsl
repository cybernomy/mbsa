...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- ********************************************************************
       $Id: pagesetup.xsl,v 1.1 2005/02/05 11:59:03 krivopoustov Exp $
       ********************************************************************
  ...
  <xsl:template name="header.footer.width">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:message>Error: value in <xsl:value-of select="$location"/>.column.widths at position
          <xsl:value-of select="$position"/> is not a number.
        </xsl:message>
        <xsl:text>1</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
