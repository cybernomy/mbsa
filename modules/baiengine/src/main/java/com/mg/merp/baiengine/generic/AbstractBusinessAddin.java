...
public abstract class AbstractBusinessAddin<T> implements BusinessAddin<T> {
  ...
  /**
   * Метод предназначен для извлечения переданных параметров, как правило переопределяется в базовых
   * классах дополнениях конкретных точек вызова, например в базовом классе настраиваемых действий
   * пользователя
   */
  ...
  /**
   * Завершить успешное выполнение алгоритма, на всех зарегистрированных слушателях будет вызван
   * метод {@link BusinessAddinListener#completed(BusinessAddinEvent)}
   *
  ...
  /**
   * Прерывает выполнение алгоритма, на всех зарегистрированных слушателях будет вызван метод {@link
   * BusinessAddinListener#aborted(BusinessAddinEvent)}
   */
  public void abort() {
