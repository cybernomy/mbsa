...
public interface SearchHelp extends Serializable {
  ...
  /**
   * запуск механизма поиска, данный метод можно использовать если реализованы средства поиска
   * сущностей по указанной, например реализовать форму поиска в которой происходит позиционирование
   * на строку отображающую переданную сущность
   *
  ...
  /**
   * устанавливает контекст импорта, значения импортируются из модели контроллера формы вызвавшей
   * данный поиск
   *
  ...
  /**
   * устанавливает контекст экспорта, значения экспортируются в модель контроллера формы вызвавшей
   * данный поиск
   */
