...
/**
 * Реализация механизма поиска сущностей, базовый абстрактный класс, в наследнике обязательно
 * необходимо переопределить как минимум один из методов {@link #doSearch()} или {@link
 * #doSearch(PersistentObject)}. Если ни один из методов не будет переопределен, то возникнет
 * зацикливание.
 *
...
public abstract class AbstractSearchHelp implements SearchHelp, SearchHelpListener {
  ...
  /**
   * выполняет поиск, должен быть переопределен в наследнике если не переопределен метод {@link
   * #doSearch(PersistentObject)}
   */
  ...
  /**
   * выполняет интерактивный просмотр, должен быть переопределен в наследнике если поддерживается
   * просмотр
   *
   * @param entity сущность
   */
  ...
  /**
   * вызывается непосредственно после выполнения поиска, наследник может реализовать дополнительную
   * функциональность, например установить контекст экспорта
   *
