...
/**
 * Стандартная форма для отображения списков бизнес-компонетов имеющих связь с иерархической
 * структурой
 *
...
public class DefaultHierarchyBrowseForm extends DefaultPlainBrowseForm {
  ...
  /**
   * загрузка модели иерархической структуры, должен быть переопределен в наследнике <p> Например:
   * <blockquote><pre>
   *   List<CatalogFolder> list = OrmTemplate.getInstance().find(CatalogFolder.class, "from
   * CatalogFolder cf where".concat(DatabaseUtils.generateFlatBrowseWhereEJBQL("cf.Id",
  ...
  /**
   * Метод устанавливает объект мастер. Необходимо переопределить в наследнике. <p> Например:
   * <blockquote><pre>
  ...
  /**
   * настройка прав на элементы иерархии, в стандартной реализации настройка происходит для видов
   * иерархии 0 (общие папки), необходимо переопределить если используется другой вид иерархии
   */
   ...
   * значение <literal>Folder</literal>, требует переопределения если имя атрибута отвечающего за
   * иерархию отличается от стандартного
   */
