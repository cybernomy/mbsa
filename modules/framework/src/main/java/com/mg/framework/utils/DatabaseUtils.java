...
public class DatabaseUtils {
   ...
   *
   * @return тип СУБД или <code>DBMSType.UNKNOWN</> если обнаружена не поддерживамая СУБД или
   * произошла ошибка доступа к СУБД
   * @see DBMSType
   */
  ...
  /**
   * генерация элемента <code>where</code> запроса EJBQL для выборки иерархических объектов без
   * учета структуры иерархии, но учитывая права пользователей. <p> Например:
   * <blockquote><pre>
   *   String ejbqlText = "from CatalogFolder cf where".concat(DatabaseUtils.generateFlatBrowseWhereEJBQL("cf.Id",
   * 1).concat(" order by cf.Id");
   * </pre></blockquote>
   *
  ...
  /**
   * генерация критериев для выборки иерархических объектов без учета структуры иерархии, но
   * учитывая права пользователей.
   *
   ...
   * @param firstRestriction первое ограничение, если <code>false</code>, то в результате будет
   *                         установлен суффикс <code>" and "</code>
   * @return строку элемента <code>where</code>
   */
  ...
  /**
   * генерация элемента <code>where</code> запроса EJBQL для ограничения по текстовому полю <p>
   * Например:
   * <blockquote><pre>
   *   whereText = whereText.concat(DatabaseUtils.formatEJBQLStringRestriction("cat.Code",
   * restForm.getCode(), "code", paramsName, paramsValue, false));
   * </pre></blockquote>
   *
   ...
   *
   * @param isUseHierarchy   использовать отбор по иерархии, если <code>false</code>, то будет
   *                         сгенерирован запрос без учета структуры иерархии
   * @param folderFieldName  наименование поля объекта отвечающего за связь со структурой
   * @param folderPart       вид иерархии
  ...
  /**
   * генерация элемента <code>where</code> запроса EJBQL для ограничения по полю <p> Например:
   * <blockquote><pre>
   *   whereText = whereText.concat(DatabaseUtils.formatEJBQLObjectRestriction("cat.GoodType",
   * restForm.getGoodType(), "goodType", paramsName, paramsValue, false));
   * </pre></blockquote>
   *
   * @param fieldName        имя поля по которому происходит ограничение
   * @param value            значение поля
   ...
   * @param firstRestriction       первое ограничение, если <code>false</code>, то в результате
   *                               будет установлен суффикс <code>" and "</code>
   * @return строку элемента <code>where</code>
   */
  ...
  /**
   * генерация списка полей браузера по умолчанию из строкового представления, разделителем является
   * символ '<strong>,</strong>' <p> Например:
   * <blockquote><pre>
