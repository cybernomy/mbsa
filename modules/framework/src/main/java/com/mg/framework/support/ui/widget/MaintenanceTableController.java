...
//				columns.add(new TableColumnModel(attrList[i]));
  ...
  /**
   * добавить слушателя на событие установки отмеченой записи, можем применяться например для
   * создание связи мастер-деталь, где мастером служит текущая таблица
   *
  ...
  /**
   * поиск и позиционирование в таблице по значению первичного ключа, если в модели найден ряд то
   * производится позиционирование на данный ряд
   *
  ...
  /**
   * установка признака выделения первого ряда таблицы при получении события смены мастера, в
   * некоторых случаях не требуется автоматическое позиционирование (например при запуске поискового
   * списка, где позиционирование происходит путем поиска по первичному ключу)
   *
