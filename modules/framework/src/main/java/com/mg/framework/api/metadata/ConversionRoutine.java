...
public interface ConversionRoutine<S, U> {
  ...
  /**
   * конвертация из значения пользовательского интерфейса в значение обрабатываемое системой
   *
  ...
  /**
   * конвертация из значения обрабатываемого системой в значение для пользовательского интерфейса
   *
  ...
  /**
   * устанавливает контекст импорта, значения импортируются из модели контроллера формы текущей для
