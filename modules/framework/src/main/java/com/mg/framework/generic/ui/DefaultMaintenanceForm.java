...
/**
 * Реализация стандартной формы поддержки для бизнес-компонента. Выполняются стандартные
 * интерактивные операции над объектом-сущностью бизнес-компонента создать, изменить, просмотреть.
 *
...
public class DefaultMaintenanceForm extends AbstractForm implements MaintenanceForm {
  ...
  /**
   * признак редактирования сложного объекта (в форме редактирования находятся списки других
   * объектов которые можно изменять), данные объекты создаются в два этапа, на первом этапе
   * заполняется и становится персистентным объект-мастер, на втором этапе возможно создание
   * объектов-деталей
   */
   ...
   * после функции "добавить" редактирование объекта мастера становится невозможным, применяется в
   * объектах, для которых невозможно совместное изменение мастера и деталей
   */
  ...
  /**
   * установить режим обновления сущности, в случае если при сохранении в хранилище происходит
   * изменение сущности, например с помощью триггера СУБД, и требуется показать данные изменения в
   * форме поддержки то необходимо установить соответствующий режим обновления
   *
  ...
  /**
   * установка в формах для сложных объектах (<code>isMasterDetail == true</code>) признака "только
   * для чтения", как правило используется для предотвращения изменений свойств зависящих от
   * объекта-мастера, например запретить создание объектов-деталей если объект-мастер еще не создан.
   * Реализовано стандартное поведение: блокирование элемента {@link com.mg.framework.api.ui.widget.MaintenanceTable
   * MaintenanceTable}. В потомках возможно переопределение стандартного поведения.
   *
   * @param readOnly признак "только для чтения"
   */
  ...
  /**
   * отправка сообщения о смене модели формы поддержки, данное событие не срабатывает при функции
   * "добавить" простого объекта, сообщение будет послано для сложного объекта только после того,
   * как он станет персистентным, смотри {@link #isMasterDetail}
   *
   * @param event событие
   */
