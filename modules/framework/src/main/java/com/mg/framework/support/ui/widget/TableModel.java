...
public interface TableModel {
  ...
  /**
   * Sets the value in the cell at <code>column</code> and <code>row</code> to <code>value</code>.
   *
  ...
  /**
   * получить параметры отрисовки ячейки таблицы, данный метод будет вызван для конкретной колонки в
   * случае если номер колонки будет передан методом {@link #getCustomRenderColumns}
   *
  ...
  /**
   * получить список колонок для которых будет обрабатываться установка параметров отрисовки с
   * помощью метода {@link #getCellRenderParameters}
   *
   ...
   *
   * @param rows an array of integers containing the indices of all selected rows, or an empty array
