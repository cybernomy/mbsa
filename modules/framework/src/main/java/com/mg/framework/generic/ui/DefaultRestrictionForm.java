...
/**
 * Базовый класс формы ограничений (условий отбора). Классы наследники расширяют свойства
 * ограничений путем добавления специфичных атрибутов, как правило рекомендуется зарегистрировать
 * атрибуты с помощью метода {@link #registerRestrictionItem()}, после регистрации состояние
 * атрибута и сохранение/восстановление в профиле будет производиться автоматически. Регистрацию
 * необходимо проводить в переопределенном конструкторе контроллера.
 *
...
public abstract class DefaultRestrictionForm extends DefaultDialog implements RestrictionForm {
  ...
  /**
   * регистрация элемента ограничения, при очистке формы атрибуты будут сброшены в начальные
   * значения, если установлен признак сохранения в профиле, то значение атрибута будет сохраняться
   * и восстанавливаться при создании формы
   *
  ...
  /**
   * реализация очистки ограничений, в наследниках возможна специфическая установка начальных
   * значений
   */
