...
/**
 * Перехватчик действий над объектами сущностями, реализации данного интерфейса используются для
 * обработки событий происходящих с объектами сущностями (создание, модификация, удаление,
 * загрузка). Реализации методов перехватчика могут генерировать неконтролируемые исключительные
 * ситуации, вызывать методы объектов JNDI, JDBC, JMS, EJB. <strong>Внимание, в реализациях методов
 * данного интерфейса запрещено выполнять операции с UI, выполнять объектные запросы, вызывать
 * метододы менеджера перманентного хранилища или модифицировать отношения
 * объектов-сущностей</strong>
 *
