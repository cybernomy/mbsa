...
/**
 * Утилиты генератора отчетов. Расположены в ядре, т.к. требуются и в студии разработки и в сервере
 * приложений
 *
...
public class ReportUtils {
  ...
  /**
   * преобразование параметров BIRT в прикладные типы BAi, т.к. в BIRT типы параметров зашиты, то
   * для создания собственных типов используем строковый тип и кодируем нужную нам информацию,
   * данный метод декодирует строку и ковертирует в параметр с custom типом системы. На текущий
   * момент поддерживатся тип Entity. Также преобразуются параметры с предустановленными именами
   * {@link com.mg.framework.api.report.RptProperties#ENTITY_IDS_DATASET_PARAMETER} и {@link
