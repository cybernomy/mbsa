...
public class Logger
  ...
  /**
   * Initialize the LoggerPlugin class to use as the delegate to the logging system. This first
   * checks to see if a pluginClassName has been specified via the {@link
   * #setPluginClassName(String)} method, then the PLUGIN_CLASS_PROP system property and finally the
   * LOG4J_PLUGIN_CLASS_NAME default. If the LoggerPlugin implementation class cannot be loaded the
   * default NullLoggerPlugin will be used.
   */
   ...
   *
   * @return true if a {@link #trace(Object)} method invocation would pass the msg to the configured
   * appenders, false otherwise.
   */
  ...
  /**
   * Issue a log msg and throwable with a level of TRACE. Invokes log.log(XLevel.TRACE, message,
   * t);
   */
   ...
   *
   * @return true if a {@link #debug(Object)} method invocation would pass the msg to the configured
   * appenders, false otherwise.
   */
   ...
   *
   * @return true if a {@link #info(Object)} method invocation would pass the msg to the configured
   * appenders, false otherwise.
   */
