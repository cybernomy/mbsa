...
public interface PersistentManager {
  ...
  /**
   * получить объект сущность по первичному ключу, состояние сущности может быть с отложенной
   * загрузкой. Если экземпляр не найден в базе данных, то будет возбуждена ИС
   * EntityNotFoundException при первом обращении к состоянию сущности
   *
   * @param <T>         тип сущности
   * @param entityClass класс сущности
   * @param primaryKey  первичный ключ
  ...
  /**
   * очистить перманентный контекст, все управляемые сущности становятся отсоединенными от
   * контекста. Изменения сделанные в сущностях которые не были синхронизированны с системой
   * хранения не будут сохранены
   */
  void clear();
  ...
  /**
   * Return the underlying provider object for the EntityManager, if available. The result of this
