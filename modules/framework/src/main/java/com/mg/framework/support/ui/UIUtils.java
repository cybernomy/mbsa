...
public class UIUtils {
  ...
  /**
   * префикс текстовой ссылки указывающий на принадлежность к ресурсам, поддерживается следующая
   * нотация resource://resource_bundle
   */
   ...
   * @return локализованное сообщение, <code>text</code> если невозможно локализовать или
   * <code>null</code> если <code>text == null</code>
   */
  ...
  /**
   * обработка ИС безопасности, при возникновении ИС безопасности не показываем стандартный диалог
   * чтобы исключить визуализацию стека вызова, просто сообщаем о нарушении прав
   *
   * @param th ИС безопасности
   * @return <code>true</code> если <code>th</code> это ИС безопасности и она обработана,
  ...
  /**
   * проверка установки модального режима, используется для определения наличия видимых модальных
   * окон, если существует видимое модальное окно и сделать видимым немодальное окно, то оно не
   * будет доступным пользователю, т.е. при существовании хотя бы одного модального окна, все
   * последующие окна должны быть модальными
   *
  ...
  /**
   * получить список локализованных наименований констант перечислимого типа по текущей локали
   * пользователя
   *
   * @param <T>   тип
   ...
   * документ <code>http://localhost:8080/jmx-console</code>, то необходимо передать путь
   * <code>jmx-console</code>
   *
   ...
   *
   * @param widget   элемент пользовательского интерфейса, если <code>null</code>, то не будет
   *                 никаких действий
