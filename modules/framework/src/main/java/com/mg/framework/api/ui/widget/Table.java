...
public interface Table extends Widget, ControllableWidget {
   ...
   * </br>off </br>next_column </br>subsequent_columns </br>last_column </br>all_column
   *
   * @see #AUTO_RESIZE_OFF
  ...
  /**
   * при изменении размера столбца пространство перераспределяется только между ним и следующим за
   * ним столбцом
   */
  final static String AUTO_RESIZE_NEXT_COLUMN = "next_column";

  /**
   * при изменении размера какого-либо столбца перераспределение пространства происходит равномерно
   * между всеми следующими за ним столбцами
   */
  ...
  /**
   * атрибут устновки режима выбора, может иметь следующие значения: </br><code>singleSelection</code>
   * </br><code>singleIntervalSelection</code> </br><code>multipleIntervalSelection</code> значение
   * по умолчанию
   *
  ...
  /**
   * атрибут признак поддержки сортировки по столбцам, имеет тип <code>boolean</code>, значение по
   * умолчанию <code>true</code>.
   */
  final static String SORTED_MODEL = "sortedModel";

  /**
   * атрибут признак поддержки выделения ячеек таблицы, имеет тип <code>boolean</code>, значение по
   * умолчанию <code>false</code>.
   */
  final static String CELL_SELECTION_ENABLED = "cellSelectionEnabled";

  /**
   * атрибут признак поддержки выделения столбца таблицы, имеет тип <code>boolean</code>, значение
  ...
  /**
   * атрибут пространства между ячейками в смежных строках, имеет тип <code>Integer</code>, значение
   * по умолчанию 1
   */
  final static String ROW_MARGIN = "rowMargin";

  /**
   * атрибут признак поддержки выделения строки таблицы, имеет тип <code>boolean</code>, значение по
   * умолчанию <code>true</code>.
   */
  ...
  /**
   * атрибут признак поддержки контекстного поиска с помощью диалога поиска, имеет тип
   * <code>boolean</code>, значение по умолчанию <code>true</code>.
   */
  final static String LOOK_AHEAD_ENABLED = "lookAheadEnabled";

  /**
   * атрибут признак поддержки контекстного поиска, имеет тип <code>boolean</code>, значение по
   * умолчанию <code>true</code>.
   */
   ...
   * @return true if the cell at index <code>(row, column)</code> is selected, where the first row
   * and first column are at index 0
   * @throws IllegalArgumentException if <code>row</code> or <code>column</code> are not in the
   *                                  valid range
   */
